...
public class XcraftSigns extends JavaPlugin {
    ...
    public boolean rewardPlayer(Player player, double reward) {
        if (player != null) {
            if (reward > 0.0) {
                this.getEconomy().depositPlayer(player, reward);
                player.sendMessage(
                    this.getCheckpointPrefix() + ChatColor.DARK_AQUA + "Du erh\u00e4ltst " + ChatColor.GOLD + reward + ChatColor.DARK_AQUA + " Euronen.");
            } else if (reward < 0.0 && this.getEconomy().has(player, reward)) {
                this.getEconomy().withdrawPlayer(player, -reward);
                player.sendMessage(
                    this.getCheckpointPrefix() + ChatColor.DARK_AQUA + "Dir wurden " + ChatColor.RED + reward * -1.0 + ChatColor.DARK_AQUA + " Euronen abgezogen.");
            } else {
                return reward == 0.0;
            }
            return true;
